import { type Post, useAccount } from '@lens-protocol/react'
...
const VideoPage = ({ post, sessionId }: { post?: Post; sessionId?: string }) => {
  const isMobile = useIsMobile()
  const { isAuthenticated, account: sessionAccount } = useSession()
  const [recordingAccountAddress, setRecordingAccountAddress] = useState<string | undefined>(
    undefined
  )
  ...
  const memoizedVideo = React.useMemo(() => {
    ...
    return (
      <Player
        ...
        className="w-full"
        muted={false}
        poster={getThumbnailFromRecordingUrl(data?.streamReplayRecording?.recordingUrl)}
      />
    )
  ...
  const memoizedAsset = React.useMemo(() => {
    ...
    return <Player src={asset?.uri} className="w-full" muted={false} poster={asset?.cover} />
  }, [asset?.uri])
    ...
    <div className="h-full w-full">
      ...
      <div className="sm:mx-8 sm:mt-6 sm:mb-0 text-p-text font-semibold sm:text-base text-sm sm:p-6 m-2 p-3 gap-y-1 start-col  rounded-xl shadow-sm bg-p-hover lg:bg-s-bg">
        <div className="flex flex-row flex-wrap gap-3 items-center">
          ...
          <div className="">
            {`${
              post?.metadata?.__typename === 'LivestreamMetadata' || (!post && sessionId)
                ? 'Streamed'
                : 'Posted'
        ...
        {post ? (
          ...
          <div>
            ...
            <span className="text-sm font-normal">
              Untitled streams are those stream that are not associated with any lens post. They are
              not visible on other lens platforms. These streams are only accessible through direct
              link. This may happen if for some reason go-live page isn't successfully able to
              create a lens post while the stream was live.{' '}
            </span>
          </div>
        )}
        {/* links */}
