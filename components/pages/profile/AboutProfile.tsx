...
const AboutProfile = ({ account }: { account: Account }) => {
  ...
  return (
    <div className="sm:my-6 mx-2 sm:mx-0 sm:p-6 p-3 rounded-xl shadow-sm bg-p-hover sm:bg-s-bg">
      <div className="text-2xl font-bold mb-4">About {formatHandle(account)}</div>
      {account?.metadata?.bio && <Markup className="mt-4">{String(account?.metadata?.bio)}</Markup>}
      {/* links */}
      <div className="flex flex-row gap-x-4 mt-2 flex-wrap break-words w-full">
        {websiteLink && (
          <ProfileLink link={websiteLink} icon={<InsertLinkIcon fontSize="small" />} />
        )}
        {twitterLink && <ProfileLink link={twitterLink} icon={<TwitterIcon fontSize="small" />} />}
        {instagramLink && (
          <ProfileLink link={instagramLink} icon={<InstagramIcon fontSize="small" />} />
        )}
        {githubLink && <ProfileLink link={githubLink} icon={<GitHubIcon fontSize="small" />} />}
        {linkedInLink && (
          <ProfileLink link={linkedInLink} icon={<LinkedInIcon fontSize="small" />} />
        )}
      </div>
    </div>
  )
